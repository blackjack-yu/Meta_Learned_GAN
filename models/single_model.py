...
class SingleModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.image_labels_A = input['A_labels']
        self.image_labels_B = input['B_labels']
        #print('Labels : ', self.image_labels)
    ...
    def predict(self):
        self.real_A = Variable(self.input_A, volatile=True)
        self.real_A_gray = Variable(self.input_A_gray, volatile=True)
        print('label ', self.image_labels_A, self.image_labels_B)
        if self.opt.noise > 0:
            self.noise = Variable(torch.cuda.FloatTensor(self.real_A.size()).normal_(mean=0, std=self.opt.noise/255.))
            self.real_A = self.real_A + self.noise
        if self.opt.input_linear:
    ...
    def backward_D_A(self):
        ...
        fake_B = self.fake_B
        fake_B_face = fake_B.index_select(0, self.label_A_face_idx)
        fake_B_dark = fake_B.index_select(0, self.label_A_dark_idx)
        fake_B_rain = fake_B.index_select(0, self.label_A_rain_idx)

        real_B_face = self.real_B.index_select(0, self.label_B_face_idx)
        real_B_dark = self.real_B.index_select(0, self.label_B_dark_idx)
        real_B_rain = self.real_B.index_select(0, self.label_B_rain_idx)
        loss_face_D_A = self.backward_D_basic(self.netD_A, real_B_face, fake_B_face, True)
        loss_dark_D_A = self.backward_D_basic(self.netD_A, real_B_dark, fake_B_dark, True)
        loss_rain_D_A = self.backward_D_basic(self.netD_A, real_B_rain, fake_B_rain, True)
        #self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_B, fake_B, True)
        #loss_face_D_A = torch.where(torch.isnan(loss_face_D_A), torch.full_like(loss_face_D_A, 0), loss_face_D_A)
        #loss_dark_D_A = torch.where(torch.isnan(loss_dark_D_A), torch.full_like(loss_dark_D_A, 0), loss_dark_D_A)
        #loss_rain_D_A = torch.where(torch.isnan(loss_rain_D_A), torch.full_like(loss_rain_D_A, 0), loss_rain_D_A)
        if(len(self.label_A_face) == 0 or len(self.label_B_face) == 0):
            loss_face_D_A = torch.zeros(1, device = 'cuda')
        if(len(self.label_A_dark) == 0 or len(self.label_B_dark) == 0):
            loss_dark_D_A = torch.zeros(1, device = 'cuda')
        if(len(self.label_A_rain) == 0 or len(self.label_B_rain) == 0):
            loss_rain_D_A = torch.zeros(1, device = 'cuda')

        self.loss_D_A  = loss_face_D_A*2  + loss_dark_D_A + loss_rain_D_A*2
        #print('D_A_loss', loss_face_D_A, loss_dark_D_A, loss_rain_D_A, self.loss_D_A)      
        if(self.loss_D_A.item() != 0):
    ...
    def backward_G(self, epoch):
        ...
        self.label_B_dark = []
        self.label_B_rain = []
        for label, i in zip(self.image_labels_A, range(len(self.image_labels_A))):
            if(label == 'face'):
                self.label_A_face.append(i)
            elif(label == 'dark'):
                self.label_A_dark.append(i)
            elif(label == 'rain'):
                self.label_A_rain.append(i)
        for label, i in zip(self.image_labels_B, range(len(self.image_labels_B))):
            if(label == 'face'):
                self.label_B_face.append(i)
            elif(label == 'dark'):
                self.label_B_dark.append(i)
            elif(label == 'rain'):
                self.label_B_rain.append(i)
        self.label_A_face_idx = torch.cuda.LongTensor(self.label_A_face)
        self.label_A_dark_idx = torch.cuda.LongTensor(self.label_A_dark)
        self.label_A_rain_idx = torch.cuda.LongTensor(self.label_A_rain)
        self.label_B_face_idx = torch.cuda.LongTensor(self.label_B_face)
        self.label_B_dark_idx = torch.cuda.LongTensor(self.label_B_dark)
        self.label_B_rain_idx = torch.cuda.LongTensor(self.label_B_rain)
        #print("A : ", label_A_face,label_A_dark,  label_A_rain)
        #print("B : ", label_B_face,label_B_dark,  label_B_rain)
        if self.opt.use_wgan:
            self.loss_G_A = -pred_fake.mean()
        elif self.opt.use_ragan:
            pred_real = self.netD_A.forward(self.real_B)
            pred_real_face = pred_real.index_select(0, self.label_B_face_idx)
            pred_real_dark = pred_real.index_select(0, self.label_B_dark_idx)
            pred_real_rain = pred_real.index_select(0, self.label_B_rain_idx)

            pred_fake_face = pred_fake.index_select(0, self.label_A_face_idx)
            pred_fake_dark = pred_fake.index_select(0, self.label_A_dark_idx)
            pred_fake_rain = pred_fake.index_select(0, self.label_A_rain_idx)
            ...
            loss_face_G_A = (self.criterionGAN(pred_real_face - torch.mean(pred_fake_face), False) +
                                      self.criterionGAN(pred_fake_face - torch.mean(pred_real_face), True)) / 2
            loss_dark_G_A = (self.criterionGAN(pred_real_dark - torch.mean(pred_fake_dark), False) +
                                      self.criterionGAN(pred_fake_dark - torch.mean(pred_real_dark), True)) / 2
            loss_rain_G_A = (self.criterionGAN(pred_real_rain - torch.mean(pred_fake_rain), False) +
                                      self.criterionGAN(pred_fake_rain - torch.mean(pred_real_rain), True)) / 2  
            #loss_face_G_A = torch.where(torch.isnan(loss_face_G_A), torch.full_like(loss_face_G_A, 0), loss_face_G_A)
            #loss_dark_G_A = torch.where(torch.isnan(loss_dark_G_A), torch.full_like(loss_dark_G_A, 0), loss_dark_G_A)
            #loss_rain_G_A = torch.where(torch.isnan(loss_rain_G_A), torch.full_like(loss_rain_G_A, 0), loss_rain_G_A)
            if(len(self.label_A_face) == 0 or len(self.label_B_face) == 0):
                loss_face_G_A = torch.zeros(1, device = 'cuda')
            if(len(self.label_A_dark) == 0 or len(self.label_B_dark) == 0):
                loss_dark_G_A = torch.zeros(1, device = 'cuda')
            if(len(self.label_A_rain) == 0 or len(self.label_B_rain) == 0):
                loss_rain_G_A = torch.zeros(1, device = 'cuda')
            self.loss_G_A = loss_face_G_A*2 + loss_dark_G_A + loss_rain_G_A*2
            #print('loss', loss_face_G_A, loss_dark_G_A, loss_rain_G_A, self.loss_G_A)            
        else:
    ...
    def optimize_parameters(self, epoch):
        ...
        if not self.opt.patchD:
            ...
            self.optimizer_D_A.step()
            self.optimizer_D_P.step()
            #print('epoch %d, loss G %f, D_A %f, D_P %f' % (epoch, self.loss_G_A, self.loss_D_A, self.loss_D_P))
