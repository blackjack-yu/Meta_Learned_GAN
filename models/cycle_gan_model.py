...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.vgg > 0:
            print('opt vgg', opt.vgg)
            self.vgg_loss = networks.PerceptualLoss(opt)
            self.vgg_loss.cuda()
            self.vgg = networks.load_vgg16("./model", opt.gpu_ids)
            self.vgg.eval()
    ...
    def get_current_errors(self, epoch):
        ...
        if epoch <= 10:
            ...
            vgg = (self.loss_vgg_a.data + self.loss_vgg_b.data) / self.opt.vgg if self.opt.vgg > 0 else 0
